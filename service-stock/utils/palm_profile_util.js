...
var Util = Class.create ({
	...
	setServerUrl: function(host){
		if (host) {
			this.ACCOUNTS_URL = "https://" + host + "/palmcsext/services/deviceJ/";
			this.GEOIP_URL = "https://" + host + "/palmcsext/services/geoipJ/";
		}
		ServiceLog.log("-------------set  Server url -----------" + this.ACCOUNTS_URL);
	...
	postRequestInternal: function(methodName, requestObj, errorFuture, errorCode, THE_URL) {
		...
		if(!THE_URL){
			...
			this.urlFuture.then(function(urlFuture){
				try{
					return Foundations.Comms.AjaxCall.post(urlFuture.result.url + methodName, messageBody, headers);
				} catch (err){
			...
			ServiceLog.log("Sending regular request to:", THE_URL);
			ServiceLog.log("Sending regular request to server with method:", methodName);
			return Foundations.Comms.AjaxCall.post(THE_URL + methodName, messageBody, headers);
		}
	},
		...
		dbFuture.then(this, function() {
			...
			if (dbFuture.result.results.length > 0) {
				...
				if (acctToken.accountServerUrl) {
					this.ACCOUNTS_URL = acctToken.accountServerUrl;
					dbFuture.result = {
